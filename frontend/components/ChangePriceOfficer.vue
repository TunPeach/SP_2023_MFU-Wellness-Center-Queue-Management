      ...
      <template v-slot:default>
        <tbody>
          <tr v-for="item in prices" :key="item.service_name">
            ...
            <td class="text-center font-display">
              <v-col cols="6">
                <v-text-field style="width: 100px" suffix="฿" v-model="item.service_price"  @input="emitServicePriceChange(selectedService, selectedAgeId, selectedGenderId, item.service_price)"></v-text-field>
              </v-col>
            </td>
          </tr>
          <v-divider></v-divider>
        ...
        <tbody v-for="(item, index) in cates" :key="index">
          ...
          <tr v-for="(subItem, subIndex) in item.subservices" :key="subIndex">
            ...
            <td>
              <v-col cols="6">
                <v-text-field style="width: 100px" suffix="฿" v-model="subItem.subservice_price" @input="emitSubservicePriceChange(subItem.subservice_ID, subItem.subservice_price)"></v-text-field>
              </v-col>
            </td>
          </tr>
          <v-divider></v-divider>
...
export default {
  ...
  data() {
    return {
      ...
      selectedService: "ตรวจสุขภาพสำหรับพนักงานมหาวิทยาลัยแม่ฟ้าหลวง",
      selectedAgeId: null,
  ...
  watch: {
    selectedAgeId: async function (newAgeVal) {
      ...
      if (this.selectedService === "ตรวจสุขภาพสำหรับพนักงานมหาวิทยาลัยแม่ฟ้าหลวง") {
        if (newAgeVal === 2 && this.selectedGenderId === 1) {
          this.serviceId = 44;
        } else if (newAgeVal === 2 && this.selectedGenderId === 2) {
          this.serviceId = 45;
        } else if (newAgeVal === 1 && this.selectedGenderId === 1) {
          this.serviceId = 46;
    ...
    selectedGenderId: async function (newGenderVal) {
      ...
      if (this.selectedService === "ตรวจสุขภาพสำหรับพนักงานมหาวิทยาลัยแม่ฟ้าหลวง") {
        if (this.selectedAgeId === 2 && newGenderVal === 1) {
          this.serviceId = 44;
        } else if (this.selectedAgeId === 2 && newGenderVal === 2) {
          this.serviceId = 45;
        } else if (this.selectedAgeId === 1 && newGenderVal === 1) {
          this.serviceId = 46;
  ...
  methods: {
    ...
    if (selectedService === "ตรวจสุขภาพสำหรับพนักงานมหาวิทยาลัยแม่ฟ้าหลวง") {
