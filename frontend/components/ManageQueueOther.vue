    ...
    <v-data-table :headers="headers" :items="bookings" :search="search" class="elevation-0">
            ...
            <v-card>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    <v-col>
                      <v-text-field label="กรุณากรอก HN"></v-text-field>
                    </v-col>
                  </v-row>
                </v-container>
              </v-card-text>
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-btn icon v-bind="attrs" v-on="on"
          @click="viewInfo(item.booking_date, item.booking_time, item.HN_code)"><v-icon color="D9D9D9">mdi-file-document-outline</v-icon></v-btn>
        <v-btn icon
        ...
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn class="font-display" color="#f17223" rounded depressed
              ...
              @click="deleteItemConfirm(itemToDelete.booking_date, itemToDelete.booking_time, itemToDelete.HN_code)">ยืนยัน</v-btn>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
export default {
  ...
  methods: {
    async initialize() {
      ...
      const res = await this.$axios.get("/qmanagement/api/appbookings");
    ...
    viewInfo(booking_date, booking_time, HN_code) {
      console.log("Received parameters - Date:", booking_date, "Time:", booking_time, "HN Code:", HN_code);
    ...
    async fetchUpdateData(date, time, hnCode) {
    ...
    deleteItemConfirm(date, time, hnCode) {
      this.closeDelete()
      this.fetchUpdateData(date, time, hnCode);
    },
