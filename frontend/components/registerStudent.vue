  ...
  <div>
        ...
        <v-form v-model="valid" ref="form">
          <v-container>
            ...
            <v-row justify="left" class="pb-6">
              <v-col>
                <h2 class="font-display">ข้อมูลบัญชี</h2>
              </v-col>
            </v-row>
            ...
            <v-row>
              ...
              <v-col style="max-width: 250px">
                <h4 class="font-display">รหัสนักศึกษา</h4>
              </v-col>
              <v-col class="textfield">
                <p class="font-display">{{ stdId }}</p>
              </v-col>
            </v-row>
            ...
            <v-row>
              ...
              <v-col class="textfield">
                <p class="font-display">{{ citizenNum }}</p>
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col style="max-width: 100px"></v-col>
              <v-col style="max-width: 250px">
                <h4 class="font-display">สร้างรหัสผ่าน</h4>
              </v-col>
              <v-col class="textfield">
                <v-text-field
            ...
            <v-row>
              <v-col style="max-width: 100px"></v-col>
              <v-col style="max-width: 250px">
                <h4 class="font-display">อาชีพ</h4>
              </v-col>
              <v-col class="textfield">
                <v-text-field
                  dense
            ...
            <v-row>
              <v-col style="max-width: 100px"></v-col>
              <v-col style="max-width: 250px">
                <h4 class="font-display">ประวัติการแพ้ยา/อาหาร</h4>
              </v-col>
              <v-col class="textfield">
                <v-text-field
                  dense
                  class="font-display"
                  v-model="drug_food_allergies"
                  outlined
                  background-color="#FEFCFC"
                  :rules="df_allergiesRules"
                  style="width: 350px"
                ></v-text-field>
              </v-col>
            </v-row>
    ...
    <v-dialog v-model="registerSuccess" persistent max-width="370px">
      <v-card class="font-display">
        <v-card-title class="font-display">สร้างบัญชีสำเร็จ!</v-card-title>
        <v-card-text>
          บัญชีของท่านได้รับการยืนยันตัวตนแล้ว
          ท่านสามารถทำการเข้าสู่ระบบเพื่อใช้บริการจองคิว</v-card-text
        >
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  data: () => ({
    ...
    id_personRules: [
      ...
      (value) => /^\d+$/.test(value) || "ไม่ควรกรอกอย่างอื่นนอกจากตัวเลข",
    ...
    passwordRules: [
      ...
      (value) => !!value || "จำเป็น",
      (value) => value.length >= 6 || "ควรมีรหัสผ่านอย่างน้อย 6 หลัก",
    ],
  }),

  created() {
    this.stdId = this.$route.query.std_id;
    this.citizenNum = this.$route.query.citizen_num;

    console.log("Received STD_ID:", this.stdId);
    console.log("Received Citizen Num:", this.citizenNum);
  },
  ...
  methods: {
    ...
    if (this.$refs.form.validate()) {
      ...
      try {
        const res = await this.$axios.put(`/qmanagement/api/students/${this.$route.query.std_id}`, {
          // Your data for the signup process
          STD_ID: this.$route.query.std_id,
          citizen_num: this.$route.query.citizen_num,
          // ... other data fields
        });
        if (res.status == 200) {
      ...
      if (this.$route.query.std_id && this.$route.query.citizen_num) {
        try {
          const res = await this.$axios.put(
            `/qmanagement/api/students/${this.$route.query.std_id}`,
            {
              STD_ID: this.$route.query.std_id,
              citizen_num: this.$route.query.citizen_num,
              ...
              occupation: this.job,
              allergy: this.drug_food_allergies,
