    ...
    <v-container>
      <v-sheet class="box" style="
        ...
        <div class="user-detail">
          ...
          <p class="font-display" style="display: inline-flex; align-items: center">
            <span style="font-weight: bold; margin-right: 10px">อายุ</span> 20
          </p>
        </div>
        <div class="grid-container">
          <div class="grid-header">
            <div class="grid-column font-display">รายการเวชศาสตร์ชะลอวัย</div>
            <div class="grid-column font-display">ค่าบริการ</div>
          </div>
          <v-divider></v-divider>
          <div v-for="(subItem, subIndex) in selectedItems" :key="subIndex">
            <div class="grid-item">
              <v-row>
                <v-col>
                  <p class="font-display" style="padding-left: 40px; padding-top: 10px;">{{ subItem.subservice_name }}</p>
                </v-col>
                <v-col>
                  <p class="font-display text-right" style="padding-top: 10px; padding-right: 40px;">{{ subItem.subservice_price }}</p>
                </v-col>
              </v-row>
            </div>
          </div>
          <div class="grid-footer">
            ...
            <h4 class="grid-column font-display">
              ราคา {{ selectedItemsTotalPrice }} บาท
            </h4>
          </div>
          <div class="button-container">
            <v-btn color="#f17223" @click="bookingService()" rounded small depressed
              style="padding-left: 30px; padding-right: 30px" class="ml-auto font-display btn-custom">ยืนยัน</v-btn>
          </div>
          <v-dialog v-model="dialogOpen" max-width="500px">
            <v-card class="font-display">
              ...
              <v-card-actions class="justify-center">
                <v-btn class="font-display" color="orange" style="padding-left: 40px; padding-right: 40px" outlined
...
export default {
  ...
  methods: {
    async bookingService() {
      ...
      for (let i = 0; i < selectedSubserviceIds.length; i++) {
        const response = await this.$axios.post("/qmanagement/api/booking", {
          ...
          status_ID: 2,
          HN_code: this.hnCode,
          service_ID: selectedServiceIds[i],
          subservice_ID: selectedSubserviceIds[i],
        });
  ...
  computed: {
    ...
    selectedItemsTotalPrice() {
      ...
      return selectedItems.reduce(
        (total, item) => total + item.subservice_price,
        0
      );
    },
    hnCode() {
