  ...
  <div>
      ...
      <v-sheet class="box" style="
        ...
        <div class="user-detail font-display">
          <p class="font-display">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">HN.</span>
            {{ this.hnCode }}
          </p>
          <p class="font-display">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">ชื่อ-สกุล</span>
            {{ this.name }}
          </p>
          <p class="font-display" style="display: inline-flex; align-items: center">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">นัดวันที่</span>
            {{ selectedDateTime.date }}
          </p>
          <p class="font-display" style="display: inline-flex; align-items: center">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">เวลา</span>
            {{ selectedDateTime.time }}
          </p>
          <br />
          <p class="font-display" style="display: inline-flex; align-items: center">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">บริการ</span>
            {{ type?.type_name ?? "-" }}
          </p>
          <p class="font-display" style="display: inline-flex; align-items: center">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">เพศ</span>
            {{ gender?.gender_detail ?? "-" }}
          </p>
          <p class="font-display" style="display: inline-flex; align-items: center">
            <span class="font-display" style="font-weight: bold; margin-right: 10px">อายุ</span>
            {{ age?.age_detail ?? "-" }}
          </p>
        </div>
        <div class="grid-container">
          <div class="grid-header">
            <div class="grid-column font-display">รายการตรวจสุขภาพ</div>
            <div class="grid-column font-display">ค่าบริการ</div>
          </div>
          <v-divider></v-divider>
          <div>
            <h4 class="font-display" style="padding-left: 40px">บริการหลัก</h4>
            <div class="grid-item" v-for="(item, index) in services" :key="index">
              <v-row>
                <v-col>
                  <p class="font-display" style="padding-left: 41px">
                    {{ item.service_name }}
                  </p>
                </v-col>
                <v-col>
                  <p class="font-display text-right" style="margin-right: 50px">
                    {{ item.service_price }}
                  </p>
                </v-col>
              </v-row>
          ...
          <div class="font-display ">
            <h4 style="padding-left: 40px">บริการตรวจสุขภาพเพิ่มเติม</h4>
          ...
          <div class="button-container">
            <v-btn @click="bookingService()" rounded small depressed style="padding-left: 30px; padding-right: 30px;"
              class="ml-auto font-display btn-custom">ยืนยัน</v-btn>
          </div>
          <v-dialog v-model="dialogOpen" max-width="500px">
            <v-card>
              <v-toolbar class="btn-custom ">
              ...
              <v-card-text class="text-center" style="margin-top: 30px">
                <v-row justify="center" style="margin-bottom: 30px">
                  ...
                  <h4 class="font-display" style="font-size: 24px ">จองคิวสำเร็จ</h4>
                </v-row>
                <p class="font-display">
                  เราได้รับคำขอจองคิวของท่านแล้ว กรุณารอการอนุมัติจากผู้ดูแล<br />
                  การอนุมัติคำขอนั้นจะใช้เวลาไม่เกิน 1 ชั่วโมง
                </p>
              </v-card-text>
              <v-card-actions class="justify-center">
                <v-btn class="font-display" style="padding-left: 40px; padding-right: 40px;" color="orange" outlined
                  rounded to="home">จองคิวเพิ่ม</v-btn>
                <v-btn class="font-display btn-custom " style="padding-left: 40px; padding-right: 40px;" rounded
                  to="myqueue">เช็คสถานะคิว</v-btn>
              </v-card-actions>
            </v-card>
          </v-dialog>
        </div>
    ...
    <v-dialog v-model="showErrorAlert" persistent max-width="390px">
      <v-card class="font-display">
        ...
        <v-card-text>คุณมีบริการอื่นๆที่ได้ทำการจองไว้ในวันและเวลาที่เลือกแล้ว</v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" to="/myqueue" text @click="showErrorAlert = false">ปิด</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  async created() {
    ...
    if (this.$route.query.genderId) {
      const res = await this.$axios.get("/qmanagement/api/gender");
      const genders = res.data.response;
      console.log(genders);
      this.gender = genders.find(
        (gender) => gender.gender_ID === Number(this.$route.query.genderId)
      );
  ...
  methods: {
    async bookingService() {
      try {
        const selectedSubserviceIds = this.selectedItems.map(
          (item) => item.subservice_ID
        );
  ...
  computed: {
    selectedItemCount() {
      return this.services.length + this.selectedItems.length;
    },
    selectedItemsTotalPrice() {
      const items = [...this.services, ...this.selectedItems];
      return items.reduce((total, item) => {
        const servicePrice = item.service_price || 0;
        const subservicePrice = item.subservice_price || 0;
        const price = servicePrice + subservicePrice;
