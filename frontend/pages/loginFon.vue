        ...
        <v-sheet class="my-card" height="536" width="751">
          ...
          <v-card-text>
            <v-form class="font-display" @submit.prevent="login">
              ...
              <v-text-field class="font-display" v-model="password" :label="showPassword ? 'Password' : 'รหัสผ่าน HN'"
              ...
              <router-link to="/register" class="font-display"  style="text-decoration: none; color: #61513f">ลงทะเบียนบัญชีใหม่</router-link>
                </v-col>
              ...
              <router-link to="/forgot-password" class="font-display"  style="text-decoration: none; color: #61513f">ลืมรหัสผ่าน HN?</router-link>
                </v-col>
              </v-row>
    ...
    login() {
      if (!this.username || !this.password) {
        this.loginErrorMessage =
          "กรุณากรอกหมายเลขประจำตัวประชาชนและรหัส HN";
        this.loginFailedDialog = true;
        return;
      }
      this.$axios
        .get("/qmanagement/api/login", {
          params: {
            user: this.username,
            pass: this.password,
          },
        ...
        .then((data) => {
          if (data.data.response.length == 0) {
            ...
            window.sessionStorage.setItem("hnCode", this.password);
