                ...
                <v-data-table :headers="headers" :items="booking" :search="search" sort-by="calories" class="data-table"
                    ...
                    <template v-slot:top>
                            ...
                            <v-dialog v-model="dialogSlip" max-width="500px">
                                <v-card class="font-display">
                                    <v-card-title class="text-h5"
                                        ...
                                        <img class="images" height="70px" :src="getImageUrl(defaultImage)"
                                    ...
                                    <v-card-text>
                                        <div v-for="item in groupedSlip" :key="item.booking_ID"
                                            ...
                                            <div style="display: flex;  flex-direction: column;">
                                                ...
                                                <span>1. กรุณานำบัตรประจำตัวประชาชนและบัตรนักศึกษามาด้วย</span>
                        ...
                        <v-dialog v-model="dialogDelete" max-width="500px">
                            <v-card>
                                <v-card-title class="text-h5"
                                    style="justify-content: center;">ต้องการลบรายการจองนี้หรือไม่</v-card-title>
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-btn class="font-display" color="#f17223"
                                        style="padding-left: 40px; padding-right: 40px;" outlined rounded
                    ...
                    <template v-slot:[`item.actions`]="{ item }">
                        ...
                        <v-icon small :style="{ color: item.status_name === 'รออนุมัติ' || item.status_name === 'ปฏิเสธ' ? 'grey' : 'green' }" 
                        style="margin-right: 5px;" :disabled="item.status_name === 'รออนุมัติ'  || item.status_name === 'ปฏิเสธ'"
...
export default {
    ...
    methods: {
        async initialize() {
            ...
            const res = await this.$axios.get("/qmanagement/api/studentbooking", {
                params: {
                    stdId: this.stdId,
                },
            });
        ...
        async downloadSlip(date, time) {
            ...
            groupedSlip.forEach((item) => {
                docDefinition.content.push(
                    ...
                    {
                        text: '1. กรุณานำบัตรประจำตัวประชาชนและบัตรนักศึกษามาด้วย',
                    },
                    {
