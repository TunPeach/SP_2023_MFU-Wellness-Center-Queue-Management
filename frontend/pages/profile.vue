...
export default {
  ...
  async created() {
    try {
      const response = await this.$axios.get("/qmanagement/api/user", {
        params: {
          hnCode: this.hnCode,
        },
      });
  ...
  methods: {
    ...
    async submitForm() {
      if (this.$refs.form.validate()) {
        try {
          console.log("HN_code:", this.hnCode);
          const hnCode = this.hnCode.toString().padStart(9, "0");
          const response = await this.$axios.put(`/qmanagement/api/users/${hnCode}`, {
            name: this.name,
