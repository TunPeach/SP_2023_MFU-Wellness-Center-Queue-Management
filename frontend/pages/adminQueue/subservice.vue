  ...
  <div>
    ...
    <v-container>
      ...
      <v-stepper v-model="e1" elevation="0" class="font-display">
        ...
        <h3 class="header font-display">บริการตรวจสุขภาพเพิ่มเติม</h3>
        <h4 class="header3 font-display">
          โปรดเลือกบริการตรวจเพิ่มเติมของคุณ สามารถเลือกบริการได้มากกว่า 1
          รายการ
        </h4>
        <div class="grid-container">
          ...
          <div class="grid-item" v-for="(item, index) in objs" :key="index">
            <v-card width="100%" outlined>
              <h4 class="font-display" style="margin-top: 15px;">{{ item.category_name }}</h4>
              <div
              ...
              >
                <div style="display: flex; align-items: center">
                  <p
                    class="subservice-name font-display"
...
export default {
  ...
  async created() {
    const res = await this.$axios.get("/qmanagement/api/category");
    ...
    async fetchSubservices() {
      for (let i = 0; i < this.objs.length; i++) {
        const response = await this.$axios.get("/qmanagement/api/checkupsub", {
          params: {
            categoryId: this.objs[i].category_ID,
            serviceId: this.$route.query.serviceId,
          },
        });
        console.log(this.objs[i].category_ID);
        this.$set(this.objs[i], "subservices", response.data.response);
      }
    },
    navigateToBooking() {
      const selectedItems = [];
      ...
      this.$router.push({ path: "/adminQueue/booking_checkupsub", query: queryParams });
    },
  },
  computed: {
    selectedItemCount() {
