...
export default {
  ...
  async created() {
    ...
    const studentResponse = await this.$axios.get("/qmanagement/api/user", {
      params: {
        hnCode: this.hnCode,
      },
    });
    ...
    this.name = studentData.response[0].name;
  ...
  methods: {
    handleButtonClick(itemId) {
      if (itemId === 1) {
        this.$router.push({ path: "/checkup", query: { typeId: itemId } });
      } else if (itemId === 2) {
        this.$router.push({ path: "/vaccine", query: { typeId: itemId } });
      } else if (itemId === 3) {
        this.$router.push({ path: "/antiaging", query: { typeId: itemId } });
      }
    },
  },
};
