...
export default {
  ...
  async created() {
    try {
      const response = await this.$axios.get("/qmanagement/api/student", {
        params: {
          stdId: this.stdId,
        },
      });
      ...
      this.job = this.user.response[0].occupation;
      this.drug_food_allergies = this.user.response[0].allergy;
      ...
      this.originalData = {
        ...
        status: this.status,
        job: this.job,
        drug_food_allergies: this.drug_food_allergies,
  ...
  methods: {
    ...
    async submitForm() {
      if (this.$refs.form.validate()) {
        try {
          console.log("HN_code:", this.stdId);
          const stdId = this.stdId.toString().padStart(9, "0");
          const response = await this.$axios.put(`/qmanagement/api/student/${stdId}`, {
            name: this.name,
    ...
    cancelEdit() {
      ...
      this.job = this.originalData.job;
      this.drug_food_allergies = this.originalData.drug_food_allergies;
